...
function start_hover_mode(should_clear_frames = true) {
  ...
  if (should_clear_frames) clear_frames();
...
function showEditFrame(el, extra_args = {}) {
  ...
  const tis = get_textitems_spans(el);
  const sentence = tis
    .map((s) => `${$(s).data("text")}`.replace(/[」「\u200B]/g, ""))
    .join("");

  extra_args.lemma = lemma;
  extra_args.reading = reading;
  extra_args.sentence = sentence;
  extra_args.bookid = bookid;
  extra_args.pagenum = pagenum;
...
function select_over(e) {
  if (selection_start_el == null) return; // Not selecting
  $(".newmultiterm").removeClass("newmultiterm");
  const selected = get_selected_in_range(
    selection_start_el,
    $(this),
    "span.textitem",
  );
...
function select_ended(e) {
  ...
  if (text.length > 250) {
    alert(
      `Selections can be max length 250 chars ("${text}" is ${text.length} chars)`,
    );
    start_hover_mode();
    return;
  }

  showEditFrame(selection_start_el, {
    textparts: textparts,
    lemmaparts: lemmaparts,
...
let current_word_index = function () {
  const i = words
    .toArray()
    .findIndex((x) => parseInt(x.dataset.order) === LUTE_CURR_TERM_DATA_ORDER);
...
let set_cursor = function (newindex) {
  ...
  if (newindex < 0 || newindex >= words.size()) return;
...
function update_status_for_marked_elements(new_status) {
  let elements = $("span.kwordmarked")
    .toArray()
    .concat($("span.wordhover").toArray());
...
function increment_status_for_selected_elements(e, shiftBy) {
  ...
  Object.keys(payloads).forEach((key) => {
    ...
    newIndex = Math.max(
      0,
      Math.min(validStatuses.length - 1, originalIndex + shiftBy),
    );
