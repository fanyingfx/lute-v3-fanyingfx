...
def all_dicts():
    ...
    for dic in mdict._config["dicts"]:
        title = dic["title"]
        dicts.append({"title": title, "url": "/dict/{0}/".format(title2url(title))})
...
def search(query):
    ...
    resp = Response(
        response=dat,  # standard way to return json
        status=200,
...
@app.route(
    '/dict/<title>/<regex(".+?\."):base><regex("css|png|jpg|gif|mp3|js|wav|ogg"):ext>'
)
